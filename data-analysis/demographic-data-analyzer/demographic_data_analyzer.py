...
def calculate_demographic_data(print_data: bool = True) -> dict:
    ...
    poor = df[df["salary"] == "<=50K"].groupby(["native-country"]).size().to_frame()
    rich = df[df["salary"] == ">50K"].groupby(["native-country"]).size().to_frame()
    perc = rich / (rich + poor)

    highest_earning_country = perc.idxmax()[0]
    highest_earning_country_percentage = round(perc.max()[0] * 100, 1)
